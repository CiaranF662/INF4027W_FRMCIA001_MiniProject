            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 space-y-5">
                    ...
                    <FormField label="Product Title" required>
                        <Input value={form.title} onChange={setInput('title')} required placeholder="e.g. Vintage Levi's 501 Original Fit Jeans" className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-600" />
                    </FormField>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <FormField label="Brand" required>
                            <Input value={form.brand} onChange={setInput('brand')} required placeholder="e.g. Levi's" className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-600" />
                        </FormField>
                        <SelectField label="Category" required value={form.category} onChange={set('category')} options={FIELD_OPTIONS.category} />
                    </div>
                    <FormField label="Description">
                        <textarea
                            value={form.description} onChange={setInput('description')}
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-5">
                        ...
                        <FormField label="Size" required>
                            <Input value={form.size} onChange={setInput('size')} required placeholder="e.g. 32 or M" className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-600" />
                        </FormField>
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 space-y-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        <FormField label="Sale Price (R)" required>
                            <Input type="number" value={form.price} onChange={setInput('price')} required min="1" placeholder="450" className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-600" />
                        </FormField>
                        <FormField label="Original Price (R)">
                            <Input type="number" value={form.originalPrice} onChange={setInput('originalPrice')} min="1" placeholder="899" className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-600" />
                        </FormField>
                        <FormField label="Cost Price (R)">
                            <Input type="number" value={form.costPrice} onChange={setInput('costPrice')} min="0" placeholder="150" className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-600" />
                        </FormField>
                    </div>
                    <p className="text-xs text-slate-400">Cost price is used in the Financial Report for profit calculations.</p>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6 space-y-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        />
                        <Button type="button" onClick={addImage} variant="outline" className="shrink-0 gap-1.5">
